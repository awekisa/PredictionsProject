...
function setupTournamentPage(games: object[]) {
  cy.intercept('GET', '**/api/tournaments/1', { statusCode: 200, body: tournament });
  cy.intercept('GET', '**/api/tournaments/1/games', { statusCode: 200, body: games }).as('getGames');
...
describe('Predictions - upcoming game', () => {
  beforeEach(() => {
    setupTournamentPage([upcomingGame]);
  });

  it('shows team names for upcoming game', () => {
    cy.contains('Arsenal').should('exist');
    cy.contains('Chelsea').should('exist');
  });

  it('shows prediction input for upcoming game', () => {
...
describe('Predictions - finished game', () => {
  it('shows final score and no prediction input', () => {
    setupTournamentPage([finishedGame]);
